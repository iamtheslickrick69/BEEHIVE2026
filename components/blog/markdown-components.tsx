...
export function KeyTakeaway({ children }: { children: React.ReactNode }) {
  return (
    <div className="my-6 p-4 bg-yellow-500/10 border-l-4 border-yellow-500 rounded-r-lg flex gap-3">
      <Star className="w-5 h-5 text-yellow-500 shrink-0 mt-0.5" />
      <div className="flex-1">
        <div className="font-semibold text-yellow-500 mb-1">KEY TAKEAWAY</div>
        <div className="text-muted-foreground text-sm">{children}</div>
      </div>
    </div>
  )
...
export function WarningBox({ children }: { children: React.ReactNode }) {
  return (
    <div className="my-6 p-4 bg-orange-500/10 border-l-4 border-orange-500 rounded-r-lg flex gap-3">
      <AlertCircle className="w-5 h-5 text-orange-500 shrink-0 mt-0.5" />
      <div className="flex-1">
        <div className="font-semibold text-orange-500 mb-1">IMPORTANT</div>
        <div className="text-muted-foreground text-sm">{children}</div>
      </div>
    </div>
  )
...
export function ContactCTA() {
  return (
    <div className="my-8 p-6 bg-gradient-to-r from-primary/10 to-yellow-500/10 border border-primary/20 rounded-xl">
